...
func handleInput(w io.Writer, input string, exit chan<- struct{}) error {
	...
	case "exit":
		exit <- struct{}{}
		return nil
	case "sh": // Add case for the sh command
		return builtins.RunShell(w)
	case "bash": // Add case for the bash command
		return builtins.RunBash(w)

	case "alias":
		return builtins.AliasCommand(w, args...)
	case "bind":
		return builtins.BindCommand(w, args...)
	case "-l":
		return builtins.ListCommand(w, args...)
	}
